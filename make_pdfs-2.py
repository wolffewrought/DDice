...
('code', [('/help', 'overview of all command groups'),
          ...
          ('/feedback send quest:\u2026', 'the same, about a run you were on \u2014 the list is\nyour own runs; the GMs\u2019 card says which one'),
...
('code', [('/fight start players:@a @b npcs:Goblin, Orc', 'any number of each', 'player'),
          ...
          ('/fight add user:@a where:Next', 'bring a player into a running fight \u2014 HP from their\nsheet, initiative rolled for the record; Next puts them\nstraight after this turn, Last at the end, ' + GM, 'gm'),
...
('code', [('/quest create', 'bare, a five-field writing window opens \u2014 name,\nobjectives, lore, details and rewards as full\nparagraphs. Numeric options (merit_reward:,\nparty_size:, hard_cap:, gm_style:) ride along.\nfrom:N seeds the window from an existing quest as\na fresh draft \u2014 a template copy, unlike instance.\nWith name: given, everything stays inline as\nbefore'),
          ...
          ('/gm check migrations', 'the ledger of one-time migrations \u2014 which have run, which\nare pending, and what each did (GM)'),
          ('/gm check pages user:@a', 'rebuild character pages from scratch \u2014 every bot block\ncleared and re-posted in order; reports what would not\ndelete. Blank does everyone (GM)'),
          ...
          ('/feedback category list', 'every room feedback can land in'),
          ('/feedback category remove name:X', 'retire a custom room (GM)'),
          ...
          ('(reviews)', 'a completed run posts a \U0001F4DD review button in its own\nthread; `/quest run show` then carries the count, the\naverage \u2b50 and the latest few lines (GM)'),
...
('code', [('/npc create name:Garrick str:3 con:2 dex:2 wis:1 lck:2', 'Make a person for the world.'),
          ...
          ('/npc temp list', 'every throwaway NPC still lying about'),
          ('/npc temp keep name:X', 'make a temporary NPC permanent \u2014 it gains a page (GM)'),
          ('/npc temp clear', 'sweep every temporary NPC away now (GM)'),
